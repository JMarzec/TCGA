...
################################################################################
#
#   Description: Pipeline for downloading HARMONIZED transcriptome profiling DATA from TCGA ( https://cancergenome.nih.gov/ ) and TARGET ( https://ocg.cancer.gov/programs/target/research ) projects using TCGAbiolinks package ( http://bioconductor.org/packages/release/bioc/vignettes/TCGAbiolinks/inst/doc/tcgaBiolinks.html ). The script outputs normalised expression (FPKM or FPKM-UQ) or raw count matrix for RNA-seq data for user-defined tissue types along with associated clinical information. The TCGA genomic data harmonization is here: https://gdc.cancer.gov/about-data/data-harmonization-and-generation/genomic-data-harmonization-0#Overview ; and the mRNA analysis pipeline is described here: https://gdc-docs.nci.nih.gov/Data/Bioinformatics_Pipelines/Expression_mRNA_Pipeline/
#
...
#
#
#   Command line use example: R --file=./TCGAbiolinks_transcriptome_profiling_data.R --args "/Users/marzec01/data/TCGA_data/TCGA-PAAD" "TCGA-PAAD" "1,11" "FPKM-UQ"
#
#
################################################################################
...
if ( !is.na(args[7]) ) {
    ...
    #print(paste(args[7], sep=".       "))

    if ( args[7] == "counts" || args[7] == "Counts" ) {

        workflowType <- "HTSeq - Counts"

        ##### Make the results directory more specific
        ProjectDir <- paste(ProjectDir, "transcriptome_profiling/Counts", sep="/" )
        ...
        ##### Make the results directory more specific
        ProjectDir <- paste(ProjectDir, "transcriptome_profiling/FPKM", sep="/" )

    } else if ( args[7] == "FPKM-UQ" ) {

        workflowType <- "HTSeq - FPKM-UQ"

        ##### Make the results directory more specific
        ProjectDir <- paste(ProjectDir, "transcriptome_profiling/FPKM-UQ", sep="/" )
...
if ( workflowType == "HTSeq - Counts" ) {
    ...
    cat( paste( "Writing FPKM normalised expression data [", paste(ProjectName, ".exp", sep=""), "] to [", ProjectDir, "]\n", sep=" ") )
    write.table( prepare2write(data_matrix), file = paste(ProjectName, ".exp", sep="") ,sep="\t", row.names=FALSE )
    ...
    cat( paste( "Writing FPKM-UQ normalised expression data [", paste(ProjectName, ".exp", sep=""), "] to [", ProjectDir, "]\n", sep=" ") )
    write.table( prepare2write(data_matrix), file = paste(ProjectName, ".exp", sep="") ,sep="\t", row.names=FALSE )
